...
// ROS2 port: dropped unused <lidar/velodyne_feature_extraction.h> (ROS1).
#include <pcl/common/transforms.h>           //pcl::transformPointCloud
// ROS2 port: dropped <pcl_conversions/...> (only the removed fromROSMsg overload
// used it); add OpenCV directly (lidar_handler uses cv::Mat for eigen-decomp,
// previously pulled in transitively via pcl_conversions).
...
namespace cocolic
{
  ...
  // Note
  // ROS2 port: dropped the cocolic::feature_cloud::ConstPtr overload (ROS1
  // message ingest, unused — the production path uses the int64_t/LiDARFeature
  // overloads below).
