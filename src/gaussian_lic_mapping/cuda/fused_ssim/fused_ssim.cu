...
__device__ void multiply_shared_mem2(float pix1[SY][SSX], float pix2[SY][SSX]) {
  auto block = cg::this_thread_block();
  const int cnt = SY * SX;
  const int num_blocks = (cnt + BX * BY - 1) / (BX * BY);
  for (int b = 0; b < num_blocks; ++b) {
...
)
{
  ...
  for (int i = 0; i < CH; ++i) {
    ...
    // calculate sigma12
    multiply_shared_mem2(buf1, buf2);
    block.sync();
    flush_conv_scratch(buf3);
    block.sync();
    do_separable_conv_x(buf1, buf3, H, W);
...
)
{
  ...
  for (int i = 0; i < CH; ++i) {
    ...
    block.sync();
    multiply_shared_mem2(buf2, buf1);
    block.sync();
    flush_conv_scratch(buf3);
    block.sync();
    do_separable_conv_x(buf2, buf3, H, W);
...
void check_image_tensor(const torch::Tensor & image, const char * name)
{
  TORCH_CHECK(image.is_cuda(), name, " must be a CUDA tensor");
  TORCH_CHECK(image.dtype() == torch::kFloat32, name, " must be float32");
  TORCH_CHECK(image.dim() == 4, name, " must have shape [B,C,H,W]");
  TORCH_CHECK(image.size(0) > 0, name, " batch dimension must be positive");
  TORCH_CHECK(image.size(1) > 0, name, " channel dimension must be positive");
  TORCH_CHECK(image.size(2) > 0, name, " height must be positive");
  TORCH_CHECK(image.size(3) > 0, name, " width must be positive");
}

void check_same_shape(const torch::Tensor & lhs, const torch::Tensor & rhs, const char * rhs_name)
{
  TORCH_CHECK(lhs.sizes() == rhs.sizes(), rhs_name, " shape must match image1");
}
...
{
  check_image_tensor(image1, "image1");
  check_image_tensor(image2, "image2");
  check_same_shape(image1, image2, "image2");

  const auto img1 = image1.contiguous();
  const auto img2 = image2.contiguous();
  int B = img1.size(0);
  int CH = img1.size(1);
  int H = img1.size(2);
  int W = img1.size(3);
  ...
  torch::Tensor dm_dmu1 = train ? torch::zeros_like(img1).contiguous() : torch::empty({0}, img1.options());
  torch::Tensor dm_dsigma1_sq =
    train ? torch::zeros_like(img1).contiguous() : torch::empty({0}, img1.options());
  torch::Tensor dm_dsigma12 = train ? torch::zeros_like(img1).contiguous() : torch::empty({0}, img1.options());
...
)
{
  check_image_tensor(image1, "image1");
  check_image_tensor(image2, "image2");
  check_image_tensor(grad_map, "grad_map");
  check_image_tensor(dmap_dmu1, "dmap_dmu1");
  check_image_tensor(dmap_dsigma1_sq, "dmap_dsigma1_sq");
  check_image_tensor(dmap_dsigma12, "dmap_dsigma12");
  check_same_shape(image1, image2, "image2");
  check_same_shape(image1, grad_map, "grad_map");
  check_same_shape(image1, dmap_dmu1, "dmap_dmu1");
  check_same_shape(image1, dmap_dsigma1_sq, "dmap_dsigma1_sq");
  check_same_shape(image1, dmap_dsigma12, "dmap_dsigma12");

  const auto img1 = image1.contiguous();
  const auto img2 = image2.contiguous();
  const auto dL_dmap = grad_map.contiguous();
  const auto dm_dmu1 = dmap_dmu1.contiguous();
  const auto dm_dsigma1_sq = dmap_dsigma1_sq.contiguous();
  const auto dm_dsigma12 = dmap_dsigma12.contiguous();
  int B = img1.size(0);
  int CH = img1.size(1);
  int H = img1.size(2);
  int W = img1.size(3);
